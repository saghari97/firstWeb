...
+function ($) {
  ...
  var documentReadyCallbackFunc = function documentReadyCallbackFunc() {
    searchToggler();
    $(".bizfit-sticky-header .bizfit-bottom-header-wrapper").sticky({
      topSpacing: 0
    });
  };
  /* DOM ready event */
  ...
  $(document).on("added_to_cart", function (e, fragments) {
    var $new_cart = $(".bizfit-navigation-n-options .cart-icon span").text(fragments["total-cart-items"]);
  });
  $(window).on("scroll", function () {
    if ($(window).scrollTop() > 200) {
      $(".bizfit-scroll-to-top").show();
    } else {
      $(".bizfit-scroll-to-top").hide();
    }
  });
  $(window).on("scroll", function () {
  ...
  $('.bizfit-scroll-to-top').click(function () {
    $("body,html").animate({
      scrollTop: 0
    }, 100);
    return false;
  ...
  function () {
    ...
    _createClass(Starter, [{
      ...
      value: function toggleSearchWithOverlay() {
        ...
        var $searchToggler = $('.bizfit-toggle-search');
        var className = 'bizfit-search-opened';
        var $searchInput = $('.bizfit-header-search .search-field');
        /* Overlay */

        var overlayClassName = 'bizfit-search-overlay';
        ...
        var toggleSearech = function toggleSearech() {
          ...
          if (_this2.$body.hasClass(className)) {
            ...
            $('.bizfit-header-icons .bizfit-toggle-search').focus();
          }
        };

        $searchToggler.length && $searchToggler.on('click', function (event) {
          event.preventDefault();
          toggleSearech();
        });
