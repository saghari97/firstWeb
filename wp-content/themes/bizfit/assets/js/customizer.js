...
!function (t, n) {
  n.sectionConstructor["bizfit-anchor"] = n.Section.extend({
...
wp.customize.controlConstructor['bizfit-buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $('.alpha-color-control').each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: bizfitColorPalette.colorPalettes // Use the passed in palette.
      ...
      $('<div class="alpha-bizfit-color-picker-container">' + '<div class="min-click-zone click-zone"></div>' + '<div class="max-click-zone click-zone"></div>' + '<div class="alpha-slider"></div>' + '<div class="transparency"></div>' + '</div>').appendTo($container.find('.wp-picker-holder'));
...
wp.customize.controlConstructor['bizfit-dimensions'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(document).ready(function ($) {
  ...
  $('.linked').on('click', function () {
    ...
    $this.parent('.link-bizfit-dimensions').removeClass('unlinked');
  }); // Unlinked button
  ...
  $('.unlinked').on('click', function () {
    ...
    $this.parent('.link-bizfit-dimensions').addClass('unlinked');
...
wp.customize.controlConstructor['bizfit-icon'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-bizfit-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-bizfit-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor['bizfit-range'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        bizfit_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(bizfit_range_input_number_timeout);
      bizfit_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
wp.customize.controlConstructor['bizfit-slider'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
(function ($, api) {
  api.controlConstructor['bizfit-toggle'] = api.Control.extend({
    ready: function ready() {
      var control = this;
