	...
	class BizFit_Helper{
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'bizfit' ); ?>
			</a>
			<?php
		}
		...
		public static function excerpt_more( $text = false ){
			...
			if( '' != bizfit_get( 'read-more-text' ) ){				
				$more = sprintf( '<a href="%1$s" class="bizfit-read-more">%2$s<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>',
			        esc_url( get_the_permalink() ),
			        esc_html( bizfit_get( 'read-more-text' ) )
			    );
			}else{
				$more = '';
			}
		...
		*/
		...
		public static function get_comment_number(){
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="bizfit-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'bizfit' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'bizfit' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'bizfit' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function get_assets_version(){
			return bizfit_get( 'assets-version' );
		}
		...
		*/
		public static function posts_navigation( $echo = true ) {
			...
			if( !$infinity_module_active || !BizFit_Helper::is_active_plugin( 'jetpack' ) ){				
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'bizfit' ),
						'next_text' => esc_html__( 'Next', 'bizfit' ),
					)
				);
			}
		}
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'bizfit' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'bizfit' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'bizfit' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		*/
		...
		public static function the_date( $post_id = null, $status = 'posted' ) {
			...
			if( $show ):
				...
				printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'bizfit' ) : esc_html__( 'Updated on', 'bizfit' ),
					$time_tag
				);
			endif;
		}
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 			    
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php bizfit_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
				...
				<header class="entry-header">
					<?php 
						if( is_page() || is_singular() ){
							...
							$news_title = get_the_title( get_option( 'page_for_posts' ) ); ?>
							<h2 class="entry-title"><?php echo esc_html( $news_title ) ?></h2>
						<?php }elseif( is_home() ){
							# when home page template is called
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'bizfit' ), 
		...
		/**
		* Font family used in BizFit Theme
		*
		* @static
		* @access public
		* @return object
		...
		*/
		public static function get_font_family( ){
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html( 'Lato' ),
				'font-2'  => esc_html( 'Oswald' ),
				'font-3'  => esc_html( 'Montserrat' ),
				'font-4'  => esc_html( 'Roboto' ), 
				'font-5'  => esc_html( 'Raleway' ), 
				'font-6'  => esc_html( 'Playfair Display' ), 
				'font-7'  => esc_html( 'Fjalla One' ),
				'font-8'  => esc_html( 'Alegreya Sans' ),
				'font-9'  => esc_html( 'PT Sans Narrow' ),
				'font-10' => esc_html( 'Open Sans' ),
				'font-11' => esc_html( 'Poppins' ),
			));
		}
		...
		public static function get_google_font(){
			...
			$google_fonts[] = self::uglify( self::get_font( bizfit_get( 'body-font' ) ), $condition );
